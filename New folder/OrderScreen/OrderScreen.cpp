...
int main()
{
    OrderNum();
    vector <Order> order;
    vector <Order> OrderFromFile;
    OrderFromFile = readFromFile();
...
void WriteToFile(vector<Order>& order)
{
    ...
    for (i = 0; i < order.size(); i++)
    {
        myFile << order[i].OrderNum << "," << order[i].Date << "," << order[i].ItemNum << "," << order[i].ItemQuant << "," << order[i].ChildName << "," << order[i].ChildRoomNum;
    }
...
vector <Order> readFromFile() {
    ...
    cout << "\n***************************\n";
    fstream myFile("OrderFile.csv", ios::in);
    vector<Order> tempMarker;
